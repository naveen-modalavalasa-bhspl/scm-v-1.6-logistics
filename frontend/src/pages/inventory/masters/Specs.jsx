...
import {
  Button, Col, Empty, Form, Input, InputNumber, Modal, Popconfirm, Row, Select,
  Space, Spin, Switch, Table, Tabs, Tag, Tree, Typography, App as AntApp,
} from 'antd';
...
const Specs = () => {
  ...
  const loadData = async () => {
    ...
    try {
      const [specCatRes, specsRes, itemCatRes, itemSpecRes, uomCatRes, uomRes] = await Promise.all([
        api.get('/inventory/spec-categories', { params: { include_inactive: true } }),
        api.get('/inventory/specs', { params: { include_inactive: true } }),
        api.get('/inventory/categories', { params: { page_size: 1000 } }),
        api.get('/inventory/item-specs', { params: { include_inactive: true } }),
        api.get('/inventory/uom-categories'),
        api.get('/inventory/uom', { params: { page_size: 500 } }),
      ]);
  ...
  const saveCategory = async () => {
    try {
      ...
      setSubmitting(true);
      if (editingCategory) {
        await api.put(`/inventory/spec-categories/${editingCategory.id}`, values);
        message.success('Spec category updated');
      } else {
        await api.post('/inventory/spec-categories', values);
  ...
  const saveSpec = async () => {
    try {
      ...
      setSubmitting(true);
      if (editingSpec) {
        await api.put(`/inventory/specs/${editingSpec.id}`, values);
        message.success('Spec updated');
      } else {
        await api.post('/inventory/specs', values);
  ...
  const mapSpec = async () => {
    try {
      ...
      setSubmitting(true);
      const res = await api.post('/inventory/item-specs', {
                    ...
                    { title: 'Status', dataIndex: 'is_active', width: 100, render: (v) => <Tag color={v ? 'green' : 'default'}>{v ? 'ACTIVE' : 'INACTIVE'}</Tag> },
                    {
                      ...
                      render: (_, r) => (
                        <Space>
                          ...
                          <Popconfirm title="Deactivate?" onConfirm={() => deactivate(`/inventory/spec-categories/${r.id}`, 'Spec category')}>
                    ...
                    { title: 'Required', dataIndex: 'is_required', width: 100, render: (v) => (v ? 'Yes' : 'No') },
                    {
                      ...
                      render: (_, r) => (
                        <Space>
                          ...
                          <Popconfirm title="Deactivate?" onConfirm={() => deactivate(`/inventory/specs/${r.id}`, 'Spec')}>
                            <Button size="small" danger>Delete</Button>
                          </Popconfirm>
                        </Space>
                      ),
