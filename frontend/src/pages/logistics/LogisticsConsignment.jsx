...
export default function LogisticsConsignment() {
  ...
  const fetchMaterialIssues = async () => {
    try {
      ...
      // Get all material_issue_numbers that already have a consignment in draft/packed/in_transit/partially_received
      const consignedMiNumbers = new Set(
        consignments
          .filter(c => ['DRAFT', 'PACKED', 'IN_TRANSIT', 'PARTIALLY_RECEIVED'].includes(c.status))
          .map(c => c.material_issue_number)
          .filter(Boolean)
      );

      const filteredIssues = (Array.isArray(issues) ? issues.filter(i => i && i.status === 'issued') : [])
        .filter(i => !consignedMiNumbers.has(i.issue_number));
  ...
  const handleIssueSelect = async (issueId) => {
    ...
    try {
      ...
      setPackages([{
        ...
        gross_weight_kg: items.reduce((sum, i) => sum + (parseFloat(i.quantity_packed || 0) * 0.5), 0),
        length_cm: 30,
        width_cm: 20,
        height_cm: 15,
        seal_number: '',
        ...
        items: items.map(i => ({
          ...
          quantity_packed: i.qty || i.quantity || 0,
