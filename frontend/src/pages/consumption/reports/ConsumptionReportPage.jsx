...
const ConsumptionReportPage = () => {
  ...
  const fetchChartData = useCallback(async () => {
    try {
      ...
      const res = await api.get('/consumption/reports/chart', { params: queryParams });
      const data = res.data;
  ...
  const fetchReport = useCallback(
    async (params) => {
      ...
      return await api.get('/consumption/reports', { params: queryParams });
    },
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await api.get('/consumption/reports', { params: queryParams });
      const data = res.data;
      ...
      {showChart && (
        <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
          <Col xs={24} lg={reportType === 'by_category' ? 12 : 24}>
            <Card title={`${REPORT_TYPES.find((r) => r.value === reportType)?.label || ''} - Chart`} variant="borderless" size="small">
          ...
          {reportType === 'by_category' && (
            <Col xs={24} lg={12}>
              <Card title="Category Distribution" variant="borderless" size="small">
                <ResponsiveContainer width="100%" height={280}>
                  <PieChart>
                    <Pie
