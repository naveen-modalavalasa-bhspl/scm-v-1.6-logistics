...
const ProcurementNotifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      setLoading(true);
      const res = await api.get('/notifications', { params: { module: 'procurement', page_size: 100 } });
  ...
  const handleRowClick = (item) => {
    ...
    switch (item.reference_type) {
      ...
      case 'mr':
        navigate(`/procurement/material-requests/${item.reference_id}`);
        break;
      case 'quotation':
      case 'rfq':
        navigate(`/procurement/quotations/${item.reference_id}`);
        break;
      case 'purchase_order':
      case 'po':
  ...
  return (
    <div style={{ padding: '24px' }}>
      <PageHeader
        title="Procurement Alerts & Notifications"
        subtitle={`You have ${unreadCount} unread procurement warnings and vendor alerts.`}
      >
        {unreadCount > 0 && (
          <Button 
            type="primary" 
            ...
            style={{ background: '#D80048', borderColor: '#D80048', borderRadius: '6px' }}
          >
            Mark All as Read
          </Button>
        )}
      ...
      {loading ? (
        <div style={{ display: 'flex', justifyContent: 'center', padding: '100px' }}>
          <Spin size="large" tip="Retrieving procurement alerts inbox..." />
        </div>
      ) : (
            ...
            <List
              ...
              renderItem={(item) => (
                <List.Item
                  style={{
                    padding: '16px',
                    background: item.is_read ? 'transparent' : 'rgba(216, 0, 72, 0.02)',
            ...
            <Empty 
              ...
              description={
                <div style={{ padding: '24px' }}>
                  ...
                  <div>No alerts or notifications in your Procurement inbox.</div>
                </div>
              }
            />
          )}
